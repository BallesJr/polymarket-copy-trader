...
- **Automation**: Each cycle runs on GitHub Actions, triggered every 5 minutes (10 before 2026-07-22) by an external cron via `workflow_dispatch`, because GitHub's own scheduler proved unreliable for this (a 30-minute cron fired every 1 to 4 hours in practice; the cron remains as fallback). State is committed back to the repo only when something beyond the timestamp changed.
...
Live state (bankroll, open/closed positions, skips, per-position leader VWAP and copy delay) is in `data/copy_state.json`, updated after every cycle. The numbers below are a snapshot, last updated **2026-08-12**. They go stale the moment the bot runs again; if that date is old, trust the state file over this section.

### Snapshot as of 2026-08-12 (bot live since 2026-07-03)

| Period (by open date) | Trades | Realized PnL | Win rate | Avg / trade |
|---|---|---|---|---|
| Before in-play filter (07-03 to 07-22) | 191 | -$1,937 | 38.7% | -$10.14 |
| After filter + 5-min trigger (07-22 to 08-12) | 81 | +$2,108 | 46.9% | +$26.03 (t = 1.53) |

Cumulative realized PnL is back above water (+$171 on the $10,000 paper bankroll). One caveat before crediting the filter: a counterfactual replay of the 141 entries it skipped (entry at the market price at skip time, same $100 stake, 134 resolved) would have made +$1,485 as well. The copied trades beat the skipped ones by only about $15 per trade (Welch t = 0.69, not significant), so the turnaround says more about the leader's regime than about the filter. It stays because the skipped set still underperforms the copied set and skipping costs nothing, but its value is unproven. The counterfactual also flatters the skips slightly, since last-trade prices ignore the in-play spread a marketable order would actually cross.

**Regime notes** (analyses should split on these boundaries):

- before 2026-07-05 ~12:30 UTC: GitHub's unreliable scheduler, copy delays of 26 to 130 minutes;
- 2026-07-05 to 07-22: external 10-minute trigger, single-digit delays (median 7.6 min, p90 13.9);
